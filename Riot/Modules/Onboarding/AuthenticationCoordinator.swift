...
final class AuthenticationCoordinator: NSObject, AuthenticationCoordinatorProtocol {
    ...
    private var homeserverString: String?
    ...
    func start() {
        Task {
            ...
            let (loginFlowResult, registrationResult) = try await refreshServer()
            
            
            if case let .success(session) = registrationResult {
                await onSessionCreated(session: session, isAccountCreated: true)
                return
            }
    ...
    private func loginFlow(homeserverString: String) async {
        currentTask = Task {
            authenticationService.cancelPendingLoginOrRegistration()
            
            do {
                let data = try await authenticationService.loginFlow(for: homeserverString)
                
                // Valid Homeserver, add it to the history.
                // Note: we add what the user has input, data.homeServerUrlBase can be different
                rememberHomeserver(homeserverString: homeserverString)
                ...
                state.homeserverStringFromUser = homeserverString
                state.homeserverString = data.homeserverURL
...
//    private func startRegistrationFlow() async throws -> {
//
//    }
    
    private func refreshServer() async throws -> (LoginFlowResult, RegistrationResult) {
        let homeserverString = knownCustomHomeserverStrings() ?? BuildSettings.serverConfigDefaultHomeserverUrlString
        let loginFlows = try await authenticationService.loginFlow(for: homeserverString)
        let wizard = try authenticationService.registrationWizard()
        let registrationFlow = try await wizard.registrationFlow()
        
        return (loginFlows, registrationFlow)
    }
    
    private func rememberHomeserver(homeserverString: String) {
        UserDefaults.standard.set(homeserverString, forKey: "customHomeServerURL")
        knownCustomHomeserverStrings()
    }

    private func clearHomeserverHistory() {
        UserDefaults.standard.removeObject(forKey: "customHomeServerURL")
        knownCustomHomeserverStrings()
    }
    
    private func knownCustomHomeserverStrings() -> String? {
        return "matrix.org"  // UserDefaults.standard.string(forKey: "customHomeServerURL")
    }
